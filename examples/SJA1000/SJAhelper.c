...
xdata unsigned char *SJA_CS_Point = (xdata unsigned char *)SJA_BASE_ADDR;	//指针指向片外数据存储区
 ...
 * 返回值：  无
**************************************************************/
void SJAInit(unsigned char btr0, unsigned char btr1, unsigned char *filter)
{
...
unsigned char SetSJASendCmd(unsigned char cmd)
{
	...
	switch(cmd){
		...
		case 1:
			...
			break;
		case 2:
			status = SetBitMask(REG_CAN_CMR, TR_BIT | SRR_BIT);	//自发自收
			break;
		case 0xff:
...
unsigned char SJASendData(unsigned char *dataBuf, unsigned char cmd)
{
	...
	//判断SJA发送缓冲区是否锁定或正在发送
	unsigned char sr;
	sr = ReadSJAReg(REG_CAN_SR);
	if((sr & (TBS_BIT | TCS_BIT)) != (TBS_BIT | TCS_BIT))
	{
		status = 0;
	}
	else{
